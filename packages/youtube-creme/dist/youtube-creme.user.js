...
(function() {
	...
	var require_react_production_min = __commonJSMin(((exports) => {
	...
	var require_scheduler_production_min = __commonJSMin(((exports) => {
		function f(a, b) {
			var c = a.length;
			a.push(b);
			a: for (; 0 < c;) {
	...
	var require_react_dom_production_min = __commonJSMin(((exports) => {
	...
	function deepMerge(target, source) {
		for (const key of Object.keys(source)) if (source[key] && typeof source[key] === "object" && !Array.isArray(source[key])) {
	...
	function loadSettings() {
		const stored = _GM_getValue(`yte_settings`, null);
	...
	function saveSettings(newSettings) {
		_GM_setValue(`yte_settings`, newSettings);
	}
	var LoggerClass = class {
		get isDebug() {
			return loadSettings().debugMode === true;
		}
		prefix = "[YouTube Creme]";
		info(...args) {
	...
	function initObserver() {
		...
		const observer = new MutationObserver((mutations) => {
			...
			if (shouldUpdate) handleDomMutations();
		});
		const observeBody = () => {
	...
	function getMergedSelectors() {
		...
		if (settings.customSelectors && settings.customSelectors.trim() !== "") try {
	...
	var SELECTORS = getMergedSelectors();
...
[data-yte-duration-filtered] ytd-thumbnail:hover {
  opacity: 1 !important;
		...
		if (barSettings.enabled && barSettings.height !== 0) {
			...
			if (barSettings.applyToVideoPage) combinedCss += cssVideoPage;
		}
		if (barSettings.enabled && barSettings.showStatsText) combinedCss += cssTextPercentage;
	...
	function getDB() {
		...
		dbPromise = new Promise((resolve, reject) => {
			...
			request.onupgradeneeded = (event) => {
				...
				if (!db.objectStoreNames.contains(STORE_NAME)) db.createObjectStore(STORE_NAME, { keyPath: "videoId" });
			};
		...
		try {
			...
			return new Promise((resolve) => {
				...
				request.onsuccess = () => {
					...
					const maxAgeMs = (loadSettings().cacheDurationHours ?? 24) * 60 * 60 * 1e3;
					if (Date.now() - entry.timestamp > maxAgeMs) {
						db.transaction(STORE_NAME, "readwrite").objectStore(STORE_NAME).delete(videoId);
						resolve(null);
	...
	async function setCachedVideoData(videoId, videoData) {
		try {
			const db = await getDB();
			return new Promise((resolve) => {
	...
	var MAX_API_RETRIES = 10;
	var API_RETRY_DELAY_MIN_MS = 3e3;
	var API_RETRY_UNIFORM_DISTRIBUTION_WIDTH_MS = 3e3;
	var pendingApiRequests = {};
	async function fetchLikesDataFromApi(videoId) {
		const cached = await getCachedVideoData(videoId);
		if (cached) return cached;
		if (pendingApiRequests[videoId]) return new Promise((resolve) => {
	...
	location.hostname;
	var IS_SEARCH_PAGE = () => location.pathname === "/results";
	var IS_WATCH_PAGE = () => location.pathname === "/watch";
	var IS_DARK_THEME = () => {
		const bgColor = getComputedStyle(document.body).getPropertyValue("--yt-spec-general-background-a");
	...
	function parseViewCount(viewText) {
		if (!viewText || /no views/i.test(viewText)) return 0;
		const match = viewText.replace(/,/g, "").match(/(\d+(?:\.\d+)?)\s*([KMBkmb])?\s*views?/i);
		...
		if (suffix && multiplier[suffix]) count *= multiplier[suffix];
	...
	function getViewCountFromContainer(container) {
		...
		const viewMatch = (container.textContent || "").match(/(\d+(?:\.\d+)?[KMBkmb]?\s*views?)/i);
		if (viewMatch) return parseViewCount(viewMatch[1]);
		return null;
	}
	function getVideoContainerSelectors() {
		if (IS_WATCH_PAGE()) return [
			"ytd-compact-video-renderer",
			"ytd-reel-item-renderer",
			"yt-lockup-view-model",
			"ytd-rich-item-renderer"
	...
	function getVideoIdFromContainer(container) {
		const lockupDiv = container.querySelector(SELECTORS.videoId.lockup) || (container.classList.contains("yt-lockup-view-model") ? container.querySelector(SELECTORS.videoId.fallbackClass) : null);
		if (lockupDiv) {
			const match = lockupDiv.className.match(/content-id-([a-zA-Z0-9_-]+)/);
		...
		if (thumbnail) return thumbnail.getAttribute("data-yte-video-id");
	...
	function exponentialRatingWidth(rating, power) {
		return 100 * Math.pow(rating, power);
	}
	function getToolTipText(videoData) {
		return `${videoData.likes.toLocaleString()} / ${videoData.dislikes.toLocaleString()} / ${videoData.total.toLocaleString()}  •  ${videoData.rating ? (videoData.rating * 100).toFixed(1) + "%" : "N/A"}`;
	}
	function createBarElement(videoData, settings) {
		const barElement = document.createElement("yte-bar");
		if (settings.opacity !== 100) barElement.style.opacity = (settings.opacity / 100).toString();
		let ratingElement;
		if (videoData.rating == null) ratingElement = document.createElement("yte-no-rating");
		else {
			const likesWidth = settings.exponentialScaling ? exponentialRatingWidth(videoData.rating, settings.exponentialPower ?? 4) : 100 * videoData.rating;
	...
	function createPercentageElement(videoData) {
		...
		if (videoData.rating !== null) {
			...
			if (!IS_DARK_THEME()) g = Math.min(g, 255) * .85;
	...
	function getMetadataLineInfo(thumbnailElement) {
		const metadataData = IS_MOBILE_SITE ? SELECTORS.metadataLineMobile : SELECTORS.metadataLineDesktop;
		for (const [containerSel, metaLineSel, itemClasses] of metadataData) {
			...
			if (metaLine) return [metaLine, itemClasses];
		}
		return [null, null];
	}
	function addBar(thumbnailElement, videoData, settings) {
		let target = thumbnailElement;
		const innerThumbnail = thumbnailElement.querySelector(SELECTORS.innerThumbnail);
		if (innerThumbnail) target = innerThumbnail;
		target.appendChild(createBarElement(videoData, settings));
	}
	function removeBar(thumbnailElement) {
		thumbnailElement?.querySelectorAll("yte-bar").forEach((el) => el.remove());
	}
	function addPercentage(thumbnailElement, videoData) {
		...
		if (itemClasses) percentEl.className = itemClasses + " yte-percentage";
		else percentEl.className = "yte-percentage";
		metaLine.appendChild(percentEl);
	}
	function removePercentage(thumbnailElement) {
		const [metaLine] = getMetadataLineInfo(thumbnailElement);
		metaLine?.querySelectorAll(".yte-percentage").forEach((el) => el.remove());
	}
	var filteredVideoIds = new Set();
	function hideOrDim(container, settings) {
		if (settings.action === "remove") container.remove();
		else {
			...
			const touchFeedback = container.querySelector("yt-touch-feedback-shape");
			if (touchFeedback) touchFeedback.style.pointerEvents = "none";
		}
	}
	function applyFilter(container, videoData, settings) {
	...
	function applyViewsFilter(container, settings) {
		if (!settings.enabled) return false;
		...
		if (container.dataset.yteViewsChecked === "true") return false;
	...
	function processViewsFilter(settings) {
		if (!settings.enabled || settings.minViews <= 0 && settings.maxViews <= 0) return;
		const containerSelectors = SELECTORS.videoContainers.split(", ");
		for (const selector of containerSelectors) {
			const containers = document.querySelectorAll(selector);
			for (const container of containers) applyViewsFilter(container, settings);
		}
	}
	function processDurationFilter(settings) {
		if (!settings.enabled) return;
		if (settings.applyOnSearchOnly && !IS_SEARCH_PAGE()) return;
		const containerSelectors = SELECTORS.videoContainers.split(", ");
		for (const selector of containerSelectors) {
			const containers = document.querySelectorAll(selector);
			for (const container of containers) {
				container.dataset.yteDurationChecked = "true";
				const timeEl = container.querySelector("#text.ytd-thumbnail-overlay-time-status-renderer, .ytd-thumbnail-overlay-time-status-renderer #text, ytd-thumbnail-overlay-time-status-renderer span, .yt-thumbnail-overlay-badge-view-model .yt-badge-shape__text, badge-shape .yt-badge-shape__text");
				if (!timeEl) continue;
				const seconds = parseDurationToSeconds(timeEl.textContent?.trim() || "");
				if (seconds !== null && seconds < settings.minDurationSeconds) {
					...
					if (settings.action === "remove") container.remove();
						...
						const opacity = (settings.dimOpacity || .3).toString();
						container.style.opacity = opacity;
						container.style.setProperty("--yte-filtered-opacity", opacity);
	...
	function processSeenVideos(settings) {
		if (!settings.enabled) return;
		const progressIndicators = document.querySelectorAll("ytd-thumbnail-overlay-resume-playback-renderer:not([data-yte-seen-checked])");
	...
	var SettingsMenu = ({ initialSettings, onClose }) => {
		...
		const handleReset = () => {
			if (confirm("Reset all settings to defaults?")) setSettings(JSON.parse(JSON.stringify(DEFAULT_SETTINGS)));
		};
  ...
  `;
	...
	async function processThumbnail(thumbnailElement, thumbnailUrl, passedVideoId) {
		...
		const extractedVideoId = thumbnailUrl.split("/")[4];
		...
		if (prevVideoId) {
			...
			if (settings.ratingBar.enabled && settings.ratingBar.height !== 0) removeBar(thumbnailElement);
			if (settings.ratingBar.enabled && settings.ratingBar.showStatsText) removePercentage(thumbnailElement);
		}
		...
		if (container && settings.videoFilter.enabled) {
			if (applyFilter(container, videoData, settings.videoFilter)) return;
			applyViewsFilter(container, settings.videoFilter);
		}
		if (settings.ratingBar.enabled && settings.ratingBar.height !== 0) addBar(thumbnailElement, videoData, settings.ratingBar);
		const creatorDisabledLikes = videoData.likes === 0 && videoData.dislikes >= 10;
		if (settings.ratingBar.enabled && settings.ratingBar.showStatsText && videoData.rating != null && !creatorDisabledLikes) addPercentage(thumbnailElement, videoData);
	}
	function processNewThumbnails() {
		...
		thumbnails.forEach((thumbnailElement) => {
			...
			if (videoId) processThumbnail(thumbnailElement, `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`, videoId);

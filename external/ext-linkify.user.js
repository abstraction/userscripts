...
var optionsEmbedImageLabel =
  'Create an image element if the URL looks like an image file.';
var optionsEmbedImageExcludeElementLabel =
  'Exclude following elements. (CSS selector)';
...
var optionsBoundaryLeftLabel =
  'Allowed characters between the whitespace and the link. (left side)';
var optionsBoundaryRightLabel =
  'Allowed characters between the whitespace and the link. (right side)';
var optionsExcludeElementLabel =
  'Do not linkify following elements. (CSS selector)';
var optionsIncludeElementLabel =
  'Always linkify following elements. Override above. (CSS selector)';
var optionsTimeoutLabel = 'Max execution time. (ms)';
var optionsTimeoutHelp =
  'The script will terminate if it takes too long to convert the entire page.';
var optionsMaxRunTimeLabel = 'Max script run time. (ms)';
var optionsMaxRunTimeHelp =
  'If the script takes too long to run, the process would be splitted into small chunks to avoid browser freeze.';
...
function createPref(DEFAULT, sep = '/') {
  ...
  function export_() {
    ...
    keys.push('scopeList');
    return storage.getMany(keys).then((changes) => {
  ...
  function updateCache(changes, rebuildCache = false) {
    ...
    for (const [key, value] of Object.entries(changes)) {
      const [scope, realKey] = key.startsWith(`global${sep}`)
        ? ['global', key.slice(6 + sep.length)]
        : key.startsWith(`${currentScope}${sep}`)
          ? [currentScope, key.slice(currentScope.length + sep.length)]
          : [null, null];
  ...
  function setCurrentScope(newScope) {
    ...
    return storage
      .getMany(Object.keys(DEFAULT).map((k) => `${newScope}${sep}${k}`))
      .then((changes) => {
...
function messageGetter({ getMessage, DEFAULT }) {
  return (key, params) => {
...
function bindInputs(pref, inputs) {
  ...
  function updatePref(key, input) {
    ...
    if (input.nodeName === 'SELECT' && input.multiple) {
      pref.set(
        key,
        [...input.options].filter((o) => o.selected).map((o) => o.value)
      );
      return;
    }
    ...
    for (const key in change) {
      ...
      for (const field of fields.get(key)) {
        field.disabled = field.dataset.bindToValue
          ? field.dataset.bindToValue !== change[key]
          : !change[key];
      }
    }
  };
    ...
    for (const type in CONTROL_METHODS) {
      for (const el of controls.get(type)) {
        el.removeEventListener(
          CONTROL_METHODS[type][0],
          CONTROL_METHODS[type][1]
        );
      }
    }
  }
...
function createGMStorage() {
  const setValue =
    typeof GM_setValue === 'function'
      ? promisify(GM_setValue)
      : GM.setValue.bind(GM);
  const getValue =
    typeof GM_getValue === 'function'
      ? promisify(GM_getValue)
      : GM.getValue.bind(GM);
  const deleteValue =
    typeof GM_deleteValue === 'function'
      ? promisify(GM_deleteValue)
  ...
  function getMany(keys) {
    return Promise.all(
      keys.map((k) =>
        getValue(`webext-pref/${k}`).then((value) => [
          k,
  ...
  function deleteMany(keys) {
    return Promise.all(keys.map((k) => deleteValue(`webext-pref/${k}`))).then(
      () => {
        if (typeof GM_addValueChangeListener === 'function') {
          const changes = {};
...
function GM_webextPref({ default: default_, separator, css = '', ...options }) {
  ...
  const registerMenu =
    typeof GM_registerMenuCommand === 'function'
      ...
      : typeof GM !== 'undefined' && GM && GM.registerMenuCommand
  ...
  function openDialog() {
    ...
    iframe.onload = () => {
      ...
      iframe.contentDocument.querySelector('.dialog-style').textContent =
      ...
      iframe.contentDocument
        .querySelector('.webext-pref-toolbar')
        .prepend(title);
  ...
  return [
    {
      ...
      children: [
        ...
        },
        {
          ...
          learnMore:
            'https://github.com/eight04/linkify-plus-plus?tab=readme-ov-file#custom-rules'
        }
      ]
    },
    {
...
var maxLength = 24;
var chars =
  'セール佛山ಭಾರತ慈善集团在线한국ଭାରତভাৰতর八卦ישראלموقعবংল公益司网站移动我爱你москвақзнлйт联通рбгеקוםファッションストアசிங்கபூர商标店城дию新闻家電中文信国國娱乐భారత్ලංකා购物クラウドભારતभारतम्ोसंगठन餐厅络у香港食品飞利浦台湾灣手机الجزئرنیتبيپکسدغظحةڀ澳門닷컴شكგე构健康ไทย招聘фລາວみんなευλ世界書籍ഭാരതംਭਾਰਤ址넷コム游戏ö企业息صط广东இலைநதயாհայ加坡ف政务';
var table = {
  aaa: true,
  aarp: true,
  abb: true,
...
class UrlMatcher {
  ...
  *match(text) {
    var { fuzzyIp = true, ignoreMustache = false, mail = true } = this.options,
      { url, invalidSuffix, mustache } = this.regex,
      urlLastIndex,
      mustacheLastIndex;
    ...
    while ((urlMatch = url.exec(text))) {
      ...
      if (
        mustacheRange &&
        result.start < mustacheRange.end &&
        result.end >= mustacheRange.start
      ) {
        continue;
      }
      ...
      if (!result.custom) {
        ...
        if (result.path) {
          ...
          pathStrip(
            result,
            /\[\/?(b|i|u|url|img|quote|code|size|color)\].*/i,
            ''
          );
        ...
        if (!isIP(result.domain)) {
          if (
            /^(http|https|mailto)/.test(result.protocol) &&
            !inTLDS(result.domain)
          ) {
            continue;
          }
...
const EVENTS$1 = [['click', handle$1, { passive: true }]];
    ...
    for (var record of mutations) {
      for (const node of record.addedNodes) {
        if (node.nodeType === 1 && !processedNodes.has(node)) {
          ...
          processes++;
          linkify({ ...options, root: node, recursive: true }).finally(() => {
            processes--;
          });
        }
      }
...
function createOptions(pref) {
  ...
  function onlink({ link, range, content }) {
    if (
      link.childNodes[0].localName !== 'img' ||
      !options.embedImageExcludeElement
    ) {
      return;
    }
...
async function startLinkifyPlusPlus(getPref) {
  ...
  if (
    ...
    !['text/plain', 'text/html', 'application/xhtml+xml'].includes(
      document.contentType
    )
  ) {
    return;
